...
from memory_mosaics.evaluation.common_metrics_epoch import estimate_loss
...
parser.add_argument('--method',type=str,default='softmax', help='method')
parser.add_argument('--out_dir',type=str,default='results/sort', help='output directory')
...
parser.add_argument('--save_checkpoint_interval', type=int, default=10000, help='save checkpoint (on disk) interval')
...
# dataset 
parser.add_argument('--task', type=str, default="reverse", help='task: mqmtar')
parser.add_argument('--datapath', type=str, default="reverse", help='your data directory')

# training process 
parser.add_argument('--batch_size',type=int, default=8, help='batch size per gpu')
parser.add_argument('--learning_rate',type=float, default=1e-3, help='learning rate')
parser.add_argument('--max_iters', type=int, default=156250, help='training iterations')
...
parser.add_argument('--warmup_iters',type=int, default=40000, help='learning rate warmup iterations')
parser.add_argument('--lr_decay_iters',type=int, default=156250, help='learning rate decay iterations')
parser.add_argument('--min_lr',type=float, default=5e-5, help='min learning rate')
...
parser.add_argument('--n_embd', type=int, default=512, help='embedding dim')
...
torch.distributed.barrier()  # <-- makes sure other ranks wait
...
ptdtype = { "float32": torch.float32, "bfloat16": torch.bfloat16,"float16": torch.float16, "float32": torch.float32 }[args.dtype]
...
# --- TRAIN (single file) ---
train_src = os.path.join(data_dir, "train.src")
train_trg = os.path.join(data_dir, "train.trg")

train_dataset = IndexedPairDataset(
    train_src, train_trg, args.task
)

# --- VALIDATION (multiple shards) ---
# We expect:   validation_?_1024.src.idx  +  validation_?_1024.trg.idx
src_shards = sorted(glob.glob(os.path.join(data_dir, "validation_*_*.src")))
trg_shards = sorted(glob.glob(os.path.join(data_dir, "validation_*_*.trg")))

assert len(src_shards) == len(trg_shards) > 0, \
    "Mismatch or missing validation shards"

val_datasets = []
for src_file, trg_file in zip(src_shards, trg_shards):
    val_datasets.append(
        IndexedPairDataset(src_file, trg_file, args.task)
    )

# Combine all validation shards into one dataset
...
collate_fn = make_indexedpair_collate(SPECIAL[args.task]["<pad>"])
# Use DataLoader for epoch-based training (one epoch)
if ddp:
...
model = StackAssoMem(method = args.method, task = args.task, vocab_size = conf.vocab_size, n_head=conf.n_head, n_embd=conf.n_embd, n_layer=conf.n_layer, \
...
if ddp:
    model = DDP(model, device_ids=[ddp_local_rank],gradient_as_bucket_view=True, find_unused_parameters=True)
...
for epoch in range(1):  # single epoch
    # ensure DDP sampler (if used) is shuffled per epoch
    if ddp and train_sampler is not None:
        train_sampler.set_epoch(epoch)

    for X, Y, context_lengths in train_loader:
        ...
        for micro_step in range(gradient_accumulation_steps):
            ...
            with ctx:
                _, loss = model(X, Y, ctx=context_lengths)
...
if master_process:
    # best-effort final logging (losses may be defined from last eval)
    try:
        logger.info( f"total time {(time.time() - tstart)/3600:.1f} h, train loss {losses['train']:.4f}, val loss {losses['val']:.4f}")
    except Exception:
        logger.info( f"total time {(time.time() - tstart)/3600:.1f} h")
